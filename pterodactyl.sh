...
get_distribution(){
    ...
    else if [ "$lsb_dist" != "ubuntu" ] && [ "$lsb_dist" != "debian" ] && [ "$lsb_dist" != "centos" ] && [ "$lsb_dist" != "rhel" ]; then
        output "不接受的運行系統."
        output ""
        output "接受的運行系統:"
...
get_virtualization(){
    ...
    if [ "$lsb_dist" =  "ubuntu" ]; then
        ...
        apt-get -y install software-properties-common
        apt-get -y install virt-what
    else if [ "$lsb_dist" =  "fedora" ] || [ "$lsb_dist" =  "centos" ] || [ "$lsb_dist" =  "rhel" ]; then
        yum -y install virt-what
    fi
    virt_serv=$(echo $(virt-what))
    if [ "$virt_serv" = "" ]; then
        output "Virtualization: Bare Metal 已偵測."
    else if [ "$virt_serv" = "openvz lxc" ]; then
...
repositories_setup(){
    ...
    if [ "$lsb_dist" =  "ubuntu" ] || [ "$lsb_dist" =  "debian" ]; then
        ...
        if [ "$lsb_dist" =  "ubuntu" ]; then
            ...
            else if [ "$dist_version" = "18.04" ]; then
            ...
            else if [ "$dist_version" = "16.04" ]; then
                add-apt-repository -y ppa:nginx/stable
                apt-key adv --recv-keys --keyserver hkp://keyserver.ubuntu.com:80 0xF1656F24C74CD1D8
            ...
            else if [ "$dist_version" = "14.04" ]; then
...
enabled_metadata=1
EOF
        else if  [ "$lsb_dist" =  "fedora" ] && [ "$dist_version" = "28" ]; then
...
EOF
        ...
        else if  [ "$lsb_dist" =  "centos" ] && [ "$dist_version" = "7" ]; then
...
EOF
    ...
    else if  [ "$lsb_dist" =  "ubuntu" ] ||  [ "$lsb_dist" =  "debian" ]; then
...
firewall(){
    ...
    else if [ "$lsb_dist" =  "centos" ] || [ "$lsb_dist" =  "fedora" ] ||  [ "$lsb_dist" =  "rhel" ]; then
...
EOF
    ...
    if [ "$lsb_dist" =  "ubuntu" ] || [ "$lsb_dist" =  "debian" ]; then
        ...
        if [ "$installoption" = "1" ]; then
            ...
            ufw allow 443
            ufw allow 3306
        else if [ "$installoption" = "2" ]; then
            ufw allow 80
            ufw allow 8080
            ufw allow 2022
        else if [ "$installoption" = "3" ]; then
            ufw allow 80
            ufw allow 443
            ufw allow 8080
            ufw allow 2022
